...
#define ONCLICK_ON_DOWN 1
#define ONCLICK_ON_UP 1
#define DONT_TRACK_PHYSICS_IDS 0
...
void allClear(hash_map<void *,storedfunc *> &within) {
	for(hash_map<void *,storedfunc *>::iterator i = within.begin(); i != within.end(); i++)
		delete i->second;
	within.clear();
}

template <class T> void allClear(std::vector<T> &within) {
	for(int c = 0; c < within.size(); c++)
		delete within[c];
	within.clear();
}

void room_auto::clear_room_objects() {
	allClear(onCollide);
	...
	allClear(ownedMaterials);

	allClear(ownedShaders);
}
...
void room_auto::die() {
	...
	CoreServices::getInstance()->setupBasicListeners();
...
//	CoreServices::getInstance()->getMaterialManager()->reloadProgramsAndTextures(); // TODO: Reinstate
			...
			case InputEvent::EVENT_KEYDOWN: {
				...
				if (!IS_MODIFIER(code)) {
					if (code == KEY_ESCAPE) {
						...
						strftime(name, 256, PROGDIR "Screenshot %Y-%m-%d %H.%M.%S.png", timeinfo);
						ERR("SAVETO: %s\n", name);
						
						Image *i = CoreServices::getInstance()->getRenderer()->renderScreenToImage();
						i->savePNG(name);
						delete i;
					}
				}
...
void room_auto::loadMaterialsFile(const string &filename) {
	MaterialManager *materialManager = CoreServices::getInstance()->getMaterialManager();
	
	std::vector<Shader*> shaders = materialManager->loadShadersFromFile(filename);
	
	for(int s=0; s < shaders.size(); s++) {
		CoreServices::getInstance()->getResourceManager()->addResource(shaders[s]);
		materialManager->addShader(shaders[s]);
		
		ownedShaders.push_back(shaders[s]);
	}
	
	vector<Material *> materials = materialManager->loadMaterialsFromFile(filename);
	
	for(int m=0; m < materials.size(); m++) {
		materials[m]->setResourceName(materials[m]->getName());
		CoreServices::getInstance()->getResourceManager()->addResource(materials[m]);
		materialManager->addMaterial(materials[m]);
		
		ownedMaterials.push_back(materials[m]);
	}
}
...
void room_auto::loadFromFile(Object &overlay, const string &filename) {
	...
	} else if (endsWith(filename, ".mat")) {
		loadMaterialsFile(filename);
...
void room_auto::screen_has_class(string name) {
	...
	if (screen) {
		screen->rootEntity.ownsChildren = true;
...
bool roomloader::localAddChild(ScreenEntity *shape, TiXmlElement *xml, svgtype kind, const svgtransform &transform) {	
	...
	const char *id = xml->Attribute("id");
	if (id) {
		if (!PHYSICS2D || !DONT_TRACK_PHYSICS_IDS || !isStatic || string("text") == xml->Value()) { // TODO: A non-physics screen on a PHYSICS2D build shouldn't hit this either.
			owner->obj[id] = shape;
		}
