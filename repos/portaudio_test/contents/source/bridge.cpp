...
BSound::BSound() : PSound() {
	for(int w = 0; w < BPARAMCOUNT; w++) {
		for(int c = 0; c < BTONECOUNT; c++) {
			switch(w) {
				case BDT:
					_tone[w][c] = M_PI / 8.0 * (c+11);
					break;
				case BCO:
					_tone[w][c] = 1;
					break;
...
double BSound::ltone(int w, int i) {
	return tone(w,i);
}

double &BSound::tone(int w, int i) {
	return _tone[w][i];
}
void BSound::setTone(int w, int i, Number v) {
//	if (w < 0 || i < 0 || w >= BPARAMCOUNT || i >= BTONECOUNT) return; // NO SAFETY
	tone(w,i) = v;
}
						  ...
						  PaStreamCallbackFlags)
{
	...
	for(unsigned long i=0; i<framesPerBuffer; i++)
	{
		...
		for(int c = 0; c < BTONECOUNT; c++) {
			...
			double &y = tone(BY,c);
			double &t = tone(BT,c);
			double &dt = tone(BDT,c);
			double &ddt = tone(BDDT,c);
			double &mem = tone(BMEM,c);
			double &co = tone(BCO,c);
			
			if (random()/double(RANDOM_MAX) < y) {
				if (ddt <= 0)
					mem = dt;
				ddt = 4410;
			}
			...
			if (flat)
				ddt = ddt - 1;
			v += sin(t)/BTONECOUNT;
			t +=  (co * 110.0 * pow(co * 2.0,sin(flat?0:dt))) / 44100.0 * M_PI*2;
			dt += M_PI*2*x / 44100.0 / 6;
		}
