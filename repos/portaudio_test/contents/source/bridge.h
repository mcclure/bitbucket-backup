...
struct BSound : PSound {
	...
	POLYIGNORE double _tone[BPARAMCOUNT][BTONECOUNT];
	int toneCount() { return BTONECOUNT; }
	Number ltone(int w, int i);
	double &tone(int w, int i);
	void setTone(int w, int i, Number v);
};
