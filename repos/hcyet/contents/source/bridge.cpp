...
// Trie implementation taken from my "Universe" project, if anyone's keeping score
...
void MarkovModel::loadTraining(const string &_in) {
	int laststart = 0;
	string in = _in + " ";
	...
	for(int c = 0; c < in.size(); c++) {
		...
		if (ch == ' ' || ch == '\t' || ch == '\n' || ch == '\r') {
			for(int d = laststart+1; d <= c; d++) {
				string sub(in, laststart, d-laststart);
				HYPERERR("ADDING %s\n", sub.c_str());
				record[sub]++;
			}
			laststart = c+1;
