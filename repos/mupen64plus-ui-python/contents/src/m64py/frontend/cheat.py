...
class Cheat(QDialog, Ui_CheatDialog):
    ...
    def read_file(self):
        ...
        shared_data = self.parent.worker.core.config.get_path( "SharedData")
        if shared_data is not None:
            cheat_file = os.path.join(shared_data, 'mupencheat.txt')
        if cheat_file is None or not os.path.isfile(cheat_file) or not os.access(cheat_file, os.R_OK):
