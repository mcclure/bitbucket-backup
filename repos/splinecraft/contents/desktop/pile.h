...
struct stateoid { virtual void set(const pile &) = 0; };

struct elementoid { virtual void draw(const pile &) = 0; };

struct state_basic : public stateoid {
	virtual void set(const pile &);
...
struct pile : public vector<float> {
	bool useThree, useTexture, useColor; GLint texture; unsigned int baseColor;
	...
	pile(elementoid *_elementDrawer, bool _useThree = false, bool _useTexture = false, bool _useColor = false, GLint _texture = 0, unsigned int _baseColor = WHITE)
		: vector<float>(), useThree(_useThree), useTexture(_useTexture), useColor(_useColor), texture(_texture), baseColor(_baseColor), elementDrawer(_elementDrawer), stateSetter(state_basic::single) {}
