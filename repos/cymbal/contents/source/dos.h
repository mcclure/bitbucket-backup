...
struct gfxcontainer {
	...
	void pxcopy_recolor(const gfxcontainer *src, unsigned int color, int _dstx = 0, int _dsty = 0, int _srcx = 0, int _srcy = 0, int _dx = -1, int dy = -1);
	...
	void pxscroll(int _dstx = 0, int _dsty = 0);
	...
	~gfxcontainer() { delete scrollScratch; }
};
...
struct type_automaton : public automaton, public gfxcontainer {
	...
	type_automaton(bool _yflip = false, int _w = 280, int _h = 192);
	void rinse_offset();
...
struct freetype_automaton : public type_automaton {
	...
	freetype_automaton(bool _yflip = false);
