	...
	virtual void Render(Camera *targetCamera = NULL) {
		...
		if (source->gfx_dirty) {
			...
			if (!gfx_mesh) {
				...
				p->addVertex(pos[0],pos[1],0, tex[0], tex[bot]);
				p->addVertex(pos[2],pos[1],0, tex[2], tex[bot]);
				p->addVertex(pos[2],pos[3],0, tex[2], tex[top]);
				p->addVertex(pos[0],pos[3],0, tex[0], tex[top]);
...
void gfxcontainer::init(int _w, int _h) {
	delete scrollScratch; scrollScratch = NULL;
	w = _w; h = _h;
	gfx.resize(_w*_h);
}

gfxcontainer::gfxcontainer(int _w, int _h) : gfx_dirty(false), scrollScratch(NULL) {
	init(_w, _h);
	pxclear();
}
...
void gfxcontainer::pxscroll(int _dstx, int _dsty) {
	if (!scrollScratch)
		scrollScratch = new gfxcontainer(w, h);
		
	scrollScratch->pxclear();
	scrollScratch->pxcopy(this, _dstx, _dsty);
	pxcopy(scrollScratch);
}
...
void gfxcontainer::pxcopy_recolor(const gfxcontainer *src, unsigned int _color, int _dstx, int _dsty, int _srcx, int _srcy, int dx, int dy) {
	if (!fix(src,_dstx,_dsty,_srcx,_srcy,dx,dy)) return;
...
type_automaton::type_automaton(bool _yflip, int _w, int _h) : automaton(), gfxcontainer(_w, _h), display(NULL), screen_dirty(true), offset_dirty(true), offx(0), offy(0), yflip(_yflip) {
	clear();
}
...
freetype_automaton::freetype_automaton(bool _yflip) : type_automaton(_yflip), x(0),y(0),autoinv(false), cursor(true), lctrl(false), rctrl(false) {
