    ...
    err(0)
{
	...
	{ // added by mcc
		int infile = open(fontFilePath, O_RDONLY, 0);
		
		if (infile < 0) {
			err = errno;
			fprintf(stderr, "FONT FAILURE 1: %d\n", errno);
		} else { 
			struct stat sb;
			if (fstat(infile, &sb) < 0) {
				err = errno;
				fprintf(stderr, "FONT FAILURE 2: %d\n", errno);
			} else {
				int size = sb.st_size;
				FT_Byte *data = new FT_Byte[size];
				
				read(infile, data, size);
	
				// Don't do anything here.
	
				err = FT_New_Memory_Face( *FTLibrary::Instance().GetLibrary(), data, size, DEFAULT_FACE_INDEX, ftFace);
