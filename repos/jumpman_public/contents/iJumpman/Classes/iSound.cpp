...
//	printf("frames %d buffers %d channels %d size %d\n", inNumberFrames, ioData->mNumberBuffers, ioData->mBuffers[0].mNumberChannels, ioData->mBuffers[0].mDataByteSize); 
	...
	for(int c = 0; c < outNumberFrames; c++) {
		...
		slick.to(value);
		/*
		value = sin( float(x) / 44100 * M_PI * 55 * 2 ) 
			// (sin( float(x)/SPAN * M_PI * 4 ) + 1.5)
		* 0.5;
		*/
//		value /= 16;
		
		left[c] = value*SHRT_MAX;
//		SWAP16(left[c]);
		if (right)
			right[c] = value*SHRT_MAX;	
//		SWAP16(right[c]);	
	}
	
	if (THIS->mute) { /*SilenceData(ioData);*/ }
...
void soundInit() {
	audioTools.outputProc.inputProc	= PerformThru; // IT'S AN OUTPUT PROC NOT AN INPUT PROC, DAMMIT	
