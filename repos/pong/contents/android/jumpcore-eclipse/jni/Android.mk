...
JUMPCORE_SRC = $(SD)/util_display.cpp $(SD)/text_display.cpp $(SD)/glCommon.cpp $(SD)/glCommonMatrix.cpp $(SD)/controls.cpp $(SD)/color.cpp $(SD)/desktop/lodepng.cpp $(SD)/slice.cpp $(SD)/desktop/static_lib/tinyxml.cpp $(SD)/desktop/static_lib/tinyxmlerror.cpp $(SD)/desktop/static_lib/tinyxmlparser.cpp
PROGRAM_SRC = $(SD)/program.cpp $(SD)/display.cpp

# FIXME: Move desktop/ stuff which is actually shared as appropriate

LOCAL_MODULE    := libgl2jni
LOCAL_CFLAGS    := -Werror -DTARGET_ANDROID -I$(SDP) -I$(SDP)/desktop -I$(SDP)/desktop/static_lib -I$(SFDP) $(FTGL_INCLUDE)
