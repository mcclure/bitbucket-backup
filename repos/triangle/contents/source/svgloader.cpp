...
bool svgloader::loadXml(TiXmlElement *xml, const svgtransform &parent_transform) {
	...
	if (name == "g") {
		...
		if (failure) { // If didn't interpret as anything OTHER than a polygon
			...
			if (points.size()>1) {
				if (points.size() == 2) {
					cpVect a = transform.apply(points[0]), b = transform.apply(points[1]);
					cpVect c = cpvmult( cpvadd(a, b), 0.5 );
					a = cpvsub(a, c); b = cpvsub(b, c);
					ScreenLine *s = new ScreenLine(cpvToVector2(a), cpvToVector2(b));
					s->setPosition(c.x,c.y,0);
	...
	if (created) {
		...
		for(int c = 0; c < style.size()-1; c++) {
			if (style[c] == "fill") { // Assumes color in #RRGGBB format; not a valid assumption for general SVGs.
				bool success;
				color = colorFrom(style[c+1], &success);
				if (success)
					haveColor = success;
			} else if (style[c] == "fill-opacity") {
				const char *v = style[c+1].c_str();
				char *endptr = NULL;
				ERR("fill-opacity: '%s'\n", v);
				double alpha = strtod(v, &endptr);
				if (v != endptr) {
					color.a = alpha;
				}
			} else if (style[c] == "stroke-width") {
				const char *v = style[c+1].c_str();
				char *endptr = NULL;
				ERR("stroke-width: '%s'\n", v);
				double width = strtod(v, &endptr);
				if (v != endptr && createdLine) {
					((ScreenLine *)created)->setLineWidth(width);
				}
			}
		}
