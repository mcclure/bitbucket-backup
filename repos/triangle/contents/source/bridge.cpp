	...
	switch (ty) {
		...
		case LUA_TTABLE: {
			...
			while (lua_next(L, t) != 0) {
                ...
                if (child->name == "__prototype") {
                    delete(child);
                    lua_pop(L, 1); // This is class data... ignore it.
                } else {
...
void project_bridge::unfocus() {
    if ((ScreenEntity*)CoreServices::getInstance()->focusedChild) {
...
String project_bridge::oneFilePicker(String ext) {
    vector<CoreFileExtension> extensions;
    if (ext.size()) {
        CoreFileExtension extObj;
        extObj.extension = ext; extObj.description = ext;
        extensions.push_back(extObj);
    }
    vector<String> results = CoreServices::getInstance()->getCore()->openFilePicker(extensions, false);
	if (results.size()>0)
		return results[0];
	return "";
}

Texture *project_bridge::findOutTexture(String materialName, String targetId) {
	...
	for(int c = 0; material && c < material->getNumShaders(); c++) {
		ShaderBinding*binding = material->getShaderBinding(c);
		for(int d = 0; d < binding->getNumOutTargetBindings(); d++) {
			RenderTargetBinding *target = binding->getOutTargetBinding(d);
...
BSound::BSound(int _loopsize, int _symbolcount, int _statecount) : PSound(), loopsize(_loopsize), symbolcount(_symbolcount), statecount(_statecount), loopc(0), repeatc(0), noiserepeatc(0), last(0), resc(INT_MAX), pitch(random()%1000), theta(0), volume(1),
    transitions(NULL), head(_loopsize/2), state(0),
    width(0),height(0),realWidth(0),rowHeight(0),gfxTemplate(NULL),gfxScratch(NULL) {
	debug_audio_init();
    ...
    if (symbolcount > 256)
        symbolcount = 256;
    symbols = new float[symbolcount];
    for(int c = 0; c < symbolcount; c++) {
        symbols[c] = float(c)/(symbolcount-1) * 2 - 1;
    }
    loop = new unsigned char[loopsize];
    memset(loop, symbolcount/2, loopsize);
    transitions = new transition[symbolcount*statecount];
    memset(transitions, 0, symbolcount*statecount);
}
...
int BSound::getIparam(int idx) {
    if (idx >= BIPARAMCOUNT)
        return 0;
	return iparam[idx];
}

void BSound::setIparam(int idx, int v) {
    if (idx >= BIPARAMCOUNT)
        return;
	iparam[idx] = v;
}

void BSound::loopScramble() {
	for(int c = 0; c < loopsize; c++)
		loop[c] = random()%symbolcount;
}

transition &BSound::transitionFor(int state, int symbol) {
    return transitions[state*symbolcount + symbol];
}

void unload_one_transaction(lua_State *L, transition &trans, int t) {
    int temp = 0;
            
    lua_pushnil(L);  // first key
    
    luaL_checktype(L, t, LUA_TTABLE);
    temp = lua_next(L, t); // Bring up nextstate
    trans.state = lua_tonumber(L,-1) - 1;
    lua_pop(L, 1); // Pop nextstate
    
    temp = lua_next(L, t); // Bring up nextsymbol
    trans.symbol = lua_tonumber(L,-1) - 1;
    lua_pop(L, 1); // Pop nextsymbol
    
    temp = lua_next(L, t); // Bring up nexthead
    trans.head = lua_tonumber(L,-1);
    lua_pop(L, 1); // Pop nexthead
    
    lua_pop(L, 1); // Pop key
}

// Arguments: One full table to upload
// Returns: None
int BSound::upload(lua_State *L) {
	luaL_checktype(L, 2, LUA_TTABLE);
    ...
    while (lua_next(L, t) != 0) { // Iterate states
        if (readstate >= statecount) {
            ERR("Upload: Too many states (State %d < %d)", readstate, statecount);
            lua_pop(L, 2); // Pop: junk value; key
            return 0;
        }
        ...
        while (lua_next(L, t) != 0) { // Iterate symbol table
            if (readsymbol >= symbolcount) {
                ERR("Upload: Too many symbols (State %d < %d)", readsymbol, symbolcount);
                lua_pop(L, 4); // Pop: (junk value; key) x 2
                return 0;
            }
        
            transition &trans = transitionFor(readstate, readsymbol);
            
            unload_one_transaction(L, trans, t);
            
            lua_pop(L, 1); // Pop transition table
            
            luaL_checktype(L, t, LUA_TTABLE); // Don't proceed if unsafe
            
            readsymbol++;
        }
        lua_pop(L, 1); // Pop symbol table
...
// Arguments: array of symbols
int BSound::adjust(lua_State *L) {
	luaL_checktype(L, -1, LUA_TTABLE);
    
    int adjstate = lua_tonumber(L,2)-1;
    int adjsymbol = lua_tonumber(L,3)-1;
    if (adjstate >= statecount || adjsymbol >= symbolcount) {
        ERR("Adjust: Invalid state/symbol (State %d < %d, symbol %d < %d)", adjstate, statecount, adjsymbol, symbolcount);
    }
    transition &trans = transitionFor(adjstate, adjsymbol);
    
    unload_one_transaction(L, trans, -2);
    
    return 0;
}

// Arguments: int state, int symbol, table with triplet to insert
int BSound::setSymbols(lua_State *L) {
    int t = -2;
    lua_pushnil(L);  // first key
    luaL_checktype(L, -2, LUA_TTABLE);
    int idx = 0;
    while (lua_next(L, t) != 0) { // Iterate symbol table
        if (idx >= symbolcount) {
            ERR("SetSymbols: index %d is too high.", idx);
            lua_pop(L, 1); // We got a junk value. Pop it.
            break;
        }
        symbols[idx++] = lua_tonumber(L,-1);
        lua_pop(L, 1); // Pop symbol value
    }
    lua_pop(L, 1); // Pop key.
...
// Arguments: None
// Returns:
int BSound::current(lua_State *L) {
    int pushstate = state, pushsymbol = loop[loopc];
    lua_newtable(L);
    lua_pushnumber(L, 1);
    lua_pushnumber(L, pushstate+1);
    lua_settable(L, -3);
    lua_pushnumber(L, 2);
    lua_pushnumber(L, pushsymbol+1);
    lua_settable(L, -3);
    return 1;
}
						  ...
						  PaStreamCallbackFlags)
{
	...
	{
		if (resc++ >= RESOLUTION) {
			last = symbols[loop[loopc++]] * volume;

			if (loopc >= loopsize) {
				// Disruption
				noiserepeatc++;
				if (noiserepeatc >= NOISEREPEATS) {
					// RERUN!
					for(int c = 0; c < NOISESTEPS; c++) {
						int idx = random()%loopsize;
						loop[idx] = random()%symbolcount;
					}
				...
				if (repeatc >= REPEATS) {
					...
					for(int c = 0; c < STEPS; c++) {
						const transition &trans = transitionFor(state, loop[head]);
						state = trans.state;
						loop[head] = trans.symbol;
						head += trans.head;
						head = (head + loopsize)%loopsize;
					}
...
void BSound::initTexture(int _width, int _height) {
    delete[] gfxTemplate;
    delete[] gfxScratch;
    width = _width; height = _height;

    int clips = ceil( float(loopsize) / width );
    rowHeight = height/clips;
    realWidth = ceil( float(loopsize) / clips );
    
    gfxTemplate = new uint32_t[width*height];
    gfxScratch = new uint32_t[width*height];
    
    // Clear
    for(int c = 0; c < width*height; c++)
        gfxTemplate[c] = BLACK;
    // Draw boxes
    int xbase = (width-realWidth)/2;
    int x = 0; int y = 0;
    for(int c = 0; c < loopsize; c++) {
        for(int y2 = 0; y2 < rowHeight; y2++) {
            Color colorHere(0.0,0.0,0.0,1.0);
            colorHere.setColorHSV(227.0/360, (0.333333*y2)/rowHeight, (0.67*y2)/rowHeight);
            gfxTemplate[ xbase+x + (y+y2)*width ] = colorHere.getUint();
        }
        
        x++; if (x > realWidth) { x = 0; y += rowHeight; }
    }
}

Texture *BSound::oscTexture() {
    if (!gfxScratch)
        return NULL;
        
    memcpy(gfxScratch, gfxTemplate, width*height*sizeof(uint32_t));
    ...
    int lasty2 = -1;
    for(int c = 0; c < loopsize; c++) {
        int y2 = (1 + symbols[loop[c]]) * (rowHeight-1)/2.0;
        
        if (y2 >= 0 && y2 < rowHeight) {
            if (lasty2 < 0 || lasty2 == y2) {
                gfxScratch[ xbase+x + (y+y2)*width ] = WHITE;
                ...
                for(int y3 = 0; y3 < cap; y3++) {
                    gfxScratch[ xbase+x + (y+y2+ y3*dir )*width ] = WHITE;
                }
            }
            lasty2 = y2;
        }
        
        x++; if (x > realWidth) { x = 0; y += rowHeight; lasty2 = -1; }
    }

    return CoreServices::getInstance()->getRenderer()->createTexture(width, height, (char *)gfxScratch, true, false);
}
