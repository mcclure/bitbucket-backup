...
#define NAME_OF_THIS_PROGRAM "LD22"
...
struct player_info : public live_info {
	cpVect lastv;
	player_info() : live_info(C_PLAYER, C_SOLIDS), lastv(cpvzero) {}
...
struct block_info : public live_info {
	block_info() : live_info(C_WALL, C_SOLIDS|C_SEP), dragging(false), lock(0), wantp(cpvzero), wantp_s(cpvzero), draggingInto(cpvzero), updated(-2), updated_s(-2) {}
	bool dragging;
	unsigned int lock;
