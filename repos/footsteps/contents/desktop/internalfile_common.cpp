...
#if SELF_EDIT && defined(TARGET_DESKTOP) && defined(__APPLE__)
	char filename[FILENAMESIZE];
	snprintf(filename, FILENAMESIZE, fmt, arg1, arg2);
	snprintf(dst, FILENAMESIZE, "../../../media/%s", filename);
	watch_file(dst);
#else
...
// Notice this is brittle; if you change the signature of game_reset, you'll get a link error here
void game_reset();
