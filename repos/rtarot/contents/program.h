...
#define NAME_OF_THIS_PROGRAM "Reverse Tarot"
...
struct barbutton {
	...
	virtual subtexture_slice *icon() { return s; }
};
...
struct generator_bb : public barbutton {
	...
	bool blanked() { return ch.samp && ch.samp->blanked; }
	virtual string name() { return !blanked() ? ch.name() : string(); }
	virtual subtexture_slice *icon() { return !blanked() ? s : NULL; }
};
...
struct bardata : public bodydata {
	...
	bardata() : bodydata(C_BAR), border(true) {}
};
...
struct barop_bb : public barbutton { // Too simplistic?
	...
	virtual string name() { static string more = "More..."; return more; }
};

struct nextbutton_bb : public barbutton { // Too simplistic?
	nextbutton_bb(subtexture_slice *_s, unsigned int _color) : barbutton(_s,_color) {}
	virtual cpShape *click(cpVect);
	virtual string name();
	virtual subtexture_slice *icon();
};
...
struct guidedata : public bodydata {
	string name;
	guichip *holding;
	
	guidedata(string _name) : bodydata(C_GUIDE), name(_name), holding(NULL) {}
};


struct textboxdata : public bodydata {
	string contents;
	
	textboxdata(string _contents) : bodydata(C_TEXTBOX), contents(_contents) {}
};
...
string nameForTarot(int suit, int card); // Human-readable
