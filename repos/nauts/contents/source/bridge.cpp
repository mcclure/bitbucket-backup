...
PhysicsScreen *project_bridge::room_screen() {
...
void project_bridge::room_remove(ScreenEntity *obj) {
	...
	if (room->obj_name.count(obj)) {
		...
		PhysicsScreenEntity *physical = room->screen->getPhysicsByScreenEntity(obj);
		if (physical) { // FIXME: Is removeChild() now safe for both cases?
			ERR("REMOVING PHYSICS CHILD\n");
			room->screen->removePhysicsChild(obj); // TODO: TEST
		} else {
			ERR("REMOVING CHILD\n");
