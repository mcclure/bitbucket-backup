...
class roomloader : public physics_svgloader {
	room_auto *owner;
...
public:
	roomloader(room_auto *_owner, PhysicsScreen *__screen = NULL) : physics_svgloader(__screen), owner(_owner) {}
};
...
void room_auto::insert() {
	...
	svg.load(filename);
	svg.screen()->addEventListener(this, PhysicsScreenEvent::EVENT_NEW_SHAPE_COLLISION);
...
bool roomloader::loadRootXml(TiXmlElement *xml, const svgtransform &parent_transform) {
	...
	return physics_svgloader::loadRootXml(xml, parent_transform);
}

bool roomloader::addChild(ScreenEntity *shape, TiXmlElement *xml, svgtype kind, const svgtransform &transform) {
	physics_svgloader::addChild(shape, xml, kind, transform);

	const char *onCollideText = xml->Attribute("polycode:onCollide");
