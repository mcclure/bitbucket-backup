...
// Abstract out mutexes based on what's available
// TODO: Give own header file?
#ifdef PLAID_SFML
#include <SFML/System/Mutex.hpp>
typedef sf::Mutex * PLAID_MUTEX;
#define PLAID_MUTEX_CREATE PLAID_MUTEX
#define PLAID_MUTEX_LOCK(m) ((m)->lock())
#define PLAID_MUTEX_UNLOCK(m) ((m)->unlock())
#define PLAID_MUTEX_DESTROY(m) (delete (m))
#else // Assume SDL
#include <SDL/SDL.h>
typedef SDL_mutex * PLAID_MUTEX;
#define PLAID_MUTEX_CREATE SDL_CreateMutex()
#define PLAID_MUTEX_LOCK(m) SDL_LockMutex(m)
#define PLAID_MUTEX_UNLOCK(m) SDL_UnlockMutex(m)
#define PLAID_MUTEX_DESTROY(m) SDL_DestroyMutex(m)
#endif
...
namespace plaid
{
	...
	public:
		...
		static PLAID_MUTEX mutex()
		{
			static PLAID_MUTEX _mutex = PLAID_MUTEX_CREATE();
			return _mutex;
		}
		WatchLock() {PLAID_MUTEX_LOCK(mutex()); ++on;}
		~WatchLock() {--on; PLAID_MUTEX_UNLOCK(mutex());}
	};
