...
local fullscreen = true -- MAKE A INPUT OR SOMETHING?
...
screen = sdl.SDL_SetVideoMode( sw, sh, 32, fullscreen and sdl.SDL_FULLSCREEN or sdl.SDL_RESIZABLE )
...
local last_keydown = engine.sdl.SDL_GetTicks()
function since_last_keydown() 
	local now = engine.sdl.SDL_GetTicks()
	return now - last_keydown
end
 	 ...
 	 handle(
	    type, {
		  ...
		  if button == 1 then
	       ...
	       function()
			 last_keydown = engine.sdl.SDL_GetTicks()
