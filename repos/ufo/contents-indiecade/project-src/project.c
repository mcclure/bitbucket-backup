...
const double value[] = //how much a given segment's similarity is worth
{
	...
	//5.0/(8.0*8.0),		//8*8
	//5.0/(16.0*16.0),	//16*16
	//75.0/(32.0*32.0),	//32*32
	//5.0/(64.0*64.0),	//64*64
	//5.0/(128.0*128.0),	//128*128
	5.0/(4.0*4.0),		//4*4
	5.0/(8.0*8.0),		//8*8
	10.0/(16.0*16.0),	//16*16
	10.0/(32.0*32.0),	//32*32
	30.0/(64.0*64.0),	//64*64
	40.0/(128.0*128.0),	//128*128
};

EXPORT double compute_similarity(SDL_Surface *surface1, SDL_Surface *surface2)
{
	int x,y,i,a,b,q;
	double total[4], real_total, quot;
	uint32_t c1, c2;
	struct rgb_chan rgb1, rgb2;
	int start_x, start_y, width, num_segs, points;
	double h1,s1,v1,h2,s2,v2;
	
	real_total = 0;
	num_segs = 4;

#define TWOPI (3.141592653589793238462643383*2)
	for (q=0; q<6; ++q)
	{
			...
			for (b=0; b<num_segs; ++b)
			{
				...
				for (x=start_x; x<start_x+width; ++x)
					for (y=start_y; y<start_y+width; ++y)
					{
						...
						rgb2 = *(struct rgb_chan*)&c2;
						RGBtoHSV((double)rgb1.x[0]/255.0,(double)rgb1.x[1]/255.0,(double)rgb1.x[2]/255.0, &h1,&s1,&v1);
						RGBtoHSV((double)rgb2.x[0]/255.0,(double)rgb2.x[1]/255.0,(double)rgb2.x[2]/255.0, &h2,&s2,&v2);
						total[0] += sin(h1*TWOPI/360) - sin(h2*TWOPI/360);
						total[1] += cos(h1*TWOPI/360) - cos(h2*TWOPI/360);
						//if (h1-h2>180)
						//	total[0] += (h1-h2-360)/180.0;
						//else if (h1-h2<-180)
						//	total[0] += (h1-h2+360)/180.0;
						//else
						//	total[0] += (h1-h2)/180.0;
				...
				quot = ((abs(total[0])+abs(total[1]))*abs(total[2])*0.5+abs(total[3])*(2-abs(total[2])))/points;
				//if (quot>0.01f) printf("%f\n", quot);
				//if (quot<1.0)//0.00001f)
					//real_total += value[q];
				if (quot>=0.0 && quot<2.0)
					real_total += value[q]*((2.0-quot)/2.0);
			}
		num_segs*=2;
	}
...
int ai_dir = 0, ai_dir_time=1;
int dir(SDL_Surface *surface1, SDL_Surface *surface2, int x, int y)
{
	--ai_dir_time;
	if (ai_dir_time==0)
	{
		ai_dir = dir_maximising_disparity(surface1,surface2,x,y);
		ai_dir_time = 8+rand()%128;
	}
	return ai_dir;
}

int andi_brushes = 0;
#define MAX_ANDI_BRUSHES 256
void (*andi_brush_list[MAX_ANDI_BRUSHES])(SDL_Surface *);

EXPORT void add_andi_brush(void (*fn)(SDL_Surface *))
{
	if (andi_brushes==MAX_ANDI_BRUSHES) return;
	andi_brush_list[andi_brushes ] = fn;
	++andi_brushes;
}
EXPORT void reset_andi_brushes()
{
	andi_brushes = 0;
}

#define IS_ATTEMPTS 16//4
int last_weapon=0;
int time_since_it_did_shit=0;
EXPORT int increase_similarity(SDL_Surface *surface1, SDL_Surface *surface2, int x, int y)
{
	...
	for (i=0; i<IS_ATTEMPTS; ++i)
	{
		...
		new_x = x;
		new_y = y;
		//if (old_similarity>50)
		//	new_d = dir_maximising_disparity(surface1,surface2,x,y);
		//else
			new_d = dir(surface1,surface2,x,y);//min(rand()%4,rand()%4);
		new_x += dx_tab[new_d];
		new_y += dy_tab[new_d];
...
#else
#define NUM_BRUSHES 21
#endif

#define TIME_UNTIL_RANDOM_MOVE 6
		...
		if (last_weapon!=0)
		{
			...
			//positive is my brushes, negative is andi's
			if (rand()%2 && andi_brushes>0)
				n = -(rand()%andi_brushes+1);
			else
				n = rand()%NUM_BRUSHES+1;
		}

		if (n<0)
		{
			(*andi_brush_list[-n-1])(newsurf);
		}
		else switch(n)
		{
		case 0:
		...
		case 12: 
			brush_size = rand()%32+1; brush_smoosh(newsurf,x,y); break;
		case 13: 
			brush_size = rand()%32+1; weird_lightning(newsurf,x,y); break;
		case 14: 
			brush_size = rand()%32+1; brush_lightning(newsurf,x,y); break;
		case 15: 
			brush_size = rand()%32+1; square_lightning(newsurf,x,y); break;
		case 16: 
			brush_size = rand()%32+1; brush_col(newsurf,x,y); break;
		case 17:
			brush_size = rand()%32+1; brush_control(newsurf,x,y); break;
		case 18: 
			brush_size = rand()%32+1; dark_lightning(newsurf,x,y); break;
		case 19: 
			brush_size = rand()%32+1; brush_untitled(newsurf,x,y); break;
		case 20: 
			brush_size = rand()%32+1; brush_dichotomy(newsurf,x,y); break;
		}

		similarity = compute_similarity(newsurf, surface2);
		if (similarity>old_similarity//-min(0.02,(time_since_it_did_shit/64)*0.001)
			|| (i==IS_ATTEMPTS-1 && time_since_it_did_shit>TIME_UNTIL_RANDOM_MOVE)
			)// || (i==IS_ATTEMPTS-1 && similarity>old_similarity-0.05))
		{
			last_weapon = n;
			memcpy(surface1->pixels, newsurf->pixels, 4 * surface1->w * surface1->h);
			SDL_FreeSurface(newsurf);
			time_since_it_did_shit = 0;
			return new_d;
		}
	...
	//none found
	SDL_FreeSurface(newsurf);
	++time_since_it_did_shit;
	return dir(surface1,surface2,x,y);//min(rand()%2,rand()%2);
}
