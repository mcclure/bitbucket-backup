...
function Player:_init(spec)
	pull(self, {
		down = {
			...
			[engine.sdl.SDL_SCANCODE_P]=function(self) -- Nonsense cocktails
				local base = self.stage.base
				local w,h = base.w,base.h
				local max = w*h
...
--function keys
			[engine.sdl.SDL_SCANCODE_RETURN]=function(self)
				--screenshot?
				local name = string.format("slideshow/save/screenshot-%s-%d.png", os.date("%Y.%m.%d-%H.%M.%S"), engine.sdl.SDL_GetTicks())
				project.save_image(self.screenshot or self.stage.base, name)
				
				local file = io.open(slideshow_index, "a+")
	...
	-- Remap keypad
	if spec.suppressEvents then 
		self.saved = {down=self.down, pressed=self.pressed}
		self.pressed = nil
		self.down = nil
	end
	
	self:super(spec)
	self.color = self.color or engine.color_make(0xFF, 0xFF, 0x7F)
end
...
-- Change this structure to change the list of tools in the rotation or their names
local arcadetools = {
	{engine.sdl.SDL_SCANCODE_W, "MEH"},
	{engine.sdl.SDL_SCANCODE_E, "PRICK"},
	{engine.sdl.SDL_SCANCODE_R, "SLASH"},
	{engine.sdl.SDL_SCANCODE_T, "MOLLIFY"},
	{engine.sdl.SDL_SCANCODE_Y, "HYDROPLANE"},
	{engine.sdl.SDL_SCANCODE_U, "VBLANK"},
	{engine.sdl.SDL_SCANCODE_I, "ROTATE"},
	{engine.sdl.SDL_SCANCODE_O, "KICK"},
	{engine.sdl.SDL_SCANCODE_P, "STRAIN"},
	{engine.sdl.SDL_SCANCODE_A, "AGITATE"},
	{engine.sdl.SDL_SCANCODE_D, "REFACTOR"},
	{engine.sdl.SDL_SCANCODE_F, "TERMINATE"},
	{engine.sdl.SDL_SCANCODE_H, "FORK"},
	{engine.sdl.SDL_SCANCODE_J, "BOUNCE"},
	{engine.sdl.SDL_SCANCODE_K, "RUE"},
	{engine.sdl.SDL_SCANCODE_L, "SHOUT"},
	{engine.sdl.SDL_SCANCODE_V, "NEW FILE"},
	{engine.sdl.SDL_SCANCODE_B, "HBLANK"},
	{engine.sdl.SDL_SCANCODE_M, "RESCUE"},
	{engine.sdl.SDL_SCANCODE_MINUS, "MAGIC"},
	{engine.sdl.SDL_SCANCODE_UP, "PHASE IN"},
	{engine.sdl.SDL_SCANCODE_DOWN, "PHASE OUT"},
	{engine.sdl.SDL_SCANCODE_LEFT, "FEINT"},
	{engine.sdl.SDL_SCANCODE_RIGHT, "COMMIT"},
	{engine.sdl.SDL_SCANCODE_SPACE, "SPACE"}
}
...
function ArcadePlayer:_init(spec)
	pull(self, {
	})
	pull(self, spec)
end

function ArcadePlayer:adjustDisplay()
	if not self.fx then
		local cx,cy = engine.sw/2, engine.sh/2
		local offx, offy = global_run.w*global_run.scale/2, global_run.h*global_run.scale/2
		self.fx = cx-offx
		self.fy = self.playernum == 1 and (cy-offy-engine.fh-2) or (cy+offy)
	end

	local s = "Player " .. tostring(self.playernum) .. ": " .. brushnames[self.brushnum] .. ", " .. arcadetools[self.toolnum][2]
	while (#s < 36) do s = s .. "  " end
	engine.draw_string(s, self.fx, self.fy)
end

function ArcadePlayer:rotateTool()
	self.toolnum = math.random(#arcadetools)
end

-- Change this function for special brush weighting
function ArcadePlayer:rotateBrush()
	self.brushnum = math.random(9)
	self.brushsize = math.random(72)
	if self.brushnum == 7 then self.brushsize = math.ceil(self.brushsize/2) end
end

class.Arcade(Thing)

function Arcade:_init(spec)
	local playerMovement = {up=engine.sdl.SDL_SCANCODE_Q, left=engine.sdl.SDL_SCANCODE_Z,
		right=engine.sdl.SDL_SCANCODE_C, down=engine.sdl.SDL_SCANCODE_N}
		
	-- Change these structures to change the control schemes
	local controls = {
		-- Player 1
		{up=engine.sdl.SDL_SCANCODE_W, left=engine.sdl.SDL_SCANCODE_A,
			down=engine.sdl.SDL_SCANCODE_S, right=engine.sdl.SDL_SCANCODE_D,
			tool=engine.sdl.SDL_SCANCODE_Z, changetool=engine.sdl.SDL_SCANCODE_X,
			changebrush=engine.sdl.SDL_SCANCODE_C,},
		{up=engine.sdl.SDL_SCANCODE_UP, left=engine.sdl.SDL_SCANCODE_LEFT,
			down=engine.sdl.SDL_SCANCODE_DOWN, right=engine.sdl.SDL_SCANCODE_RIGHT,
			tool=engine.sdl.SDL_SCANCODE_N, changetool=engine.sdl.SDL_SCANCODE_M,
			changebrush=engine.sdl.SDL_SCANCODE_B,},
	}
	...
	for playernum=1,2 do
		local control = controls[playernum]
		local at = playernum == 2 and 96 or 32
		local player = Player({x=at, y=at, suppressEvents=true, color=playernum == 2 and engine.color_make(0x7F, 0xFF, 0xFF)})
		local arcadeplayer = ArcadePlayer({player=player, playernum=playernum})
		self.player[playernum] = arcadeplayer
		
		for _,kind in ipairs({"pressed","down"}) do
		
			-- Directions
			for movename, movekey in pairs(playerMovement) do
				local keyfunc  = player.saved[kind][movekey]
				if keyfunc then
					self[kind][ control[movename] ] = function(self)
						project.set_brush_type( arcadeplayer.brushnum )
						project.set_brush_size( arcadeplayer.brushsize )
						keyfunc(player)
					end
				end
			end
			...
			-- Splat
			self[kind][ control.tool ] = function(self)
				local keyfunc = player.saved[kind][ arcadetools[ arcadeplayer.toolnum ][1] ]
				if keyfunc then
					keyfunc(player)
				end
			end
		...
		self.pressed[ control.changetool ] = function(self)
			project.playsample_set(sampleplayer, ogg_named("sfx_5"), 1, 0)
			arcadeplayer:rotateTool()
			arcadeplayer:adjustDisplay()
		end
		self.pressed[ control.changebrush ] = function(self)
			arcadeplayer:rotateBrush()
			project.playsample_set(sampleplayer, ogg_named("brush_" .. tostring(arcadeplayer.brushnum) ), 1, 0)
			arcadeplayer:adjustDisplay()
		end
		
		-- PRINT
		self.pressed[engine.sdl.SDL_SCANCODE_RETURN] = function(self) player.saved.pressed[engine.sdl.SDL_SCANCODE_RETURN](player) end
	end
	
	self:super(spec)
end

function Arcade:gotStage(stage)
	for i=1,2 do self.player[i].player.stage = stage end
end

function Arcade:onTick()
	for i=1,2 do self.player[i].player:onTick() end
end

function Arcade:insert(prio)
	for i=1,2 do
		local player = self.player[i]
		player:rotateTool()
		player:rotateBrush()
		player:adjustDisplay()
	end
