...
function FreeRunner:_init(spec)
	...
	if not self.scale then
		...
		while true do
			...
			if tw >= engine.sw or th + (self.wordpad and (engine.fh*2) or 0) >= engine.sh then break end
			self.scale = scale
		end
	end
...
-- NO TO: W, X, E, O, A, G, S, V, UP
local player_ops = { engine.sdl.SDL_SCANCODE_Q,
	engine.sdl.SDL_SCANCODE_R, engine.sdl.SDL_SCANCODE_T, engine.sdl.SDL_SCANCODE_Y, engine.sdl.SDL_SCANCODE_U,
	engine.sdl.SDL_SCANCODE_I, engine.sdl.SDL_SCANCODE_P,
	engine.sdl.SDL_SCANCODE_D, engine.sdl.SDL_SCANCODE_F, engine.sdl.SDL_SCANCODE_K,
	engine.sdl.SDL_SCANCODE_L, engine.sdl.SDL_SCANCODE_Z, engine.sdl.SDL_SCANCODE_C, engine.sdl.SDL_SCANCODE_B,
	engine.sdl.SDL_SCANCODE_N, engine.sdl.SDL_SCANCODE_M, engine.sdl.SDL_SCANCODE_SPACE, -- NO X
	engine.sdl.SDL_SCANCODE_H, engine.sdl.SDL_SCANCODE_J,
	engine.sdl.SDL_SCANCODE_DOWN,
	engine.sdl.SDL_SCANCODE_LEFT, engine.sdl.SDL_SCANCODE_RIGHT,
}

class.NightRunner(FreeRunner)

function NightRunner:_init(spec)
	self:super(spec)
	self.current_op = null
end

function NightRunner:onTick()
	FreeRunner.onTick(self)
	if self:should_reset() and self.snapshot:time() > 7*1000 then
		if not self.current_op then
			self.current_op = player_ops[ math.random(#player_ops) ]
		end
		for i,v in pairs(self.player.actions) do
			local a = v[self.current_op]
			if a then a(self.player) break end
		end
	end
end

function NightRunner:reset()
	for _=1,100 do
		local path = slideshow [ math.random(#slideshow) ]
		print(path)
		local map = MapLoad(path)
		if map then
			MapCopy(self.stage.base, map)
			MapKill(map)
			break
		end
	end
...
function NightRunner:should_reset()
	return engine.since_last_keydown() > 20000
end

function NightRunner:insert(prio)
	local e = FreeRunner.insert(self, prio)
	
	self:reset()
	local snap = self.snapshot.snap
	self.snapshot.snap = function(snapself)
	...
	local playerTick = self.player.onTick
	self.player.onTick = function(playerself)
		if not self:should_reset() then
			playerTick(playerself)
		end
	end
