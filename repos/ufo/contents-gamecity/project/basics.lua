...
function Runner:insert(prio)
	...
	self.player = self.player or ( self.playerMaker and self:playerMaker() )
		or Player({x=32, y=32, stage=self.stage, blip=self.cursor_blip, screenshot=self.screenshot})
...
function MapConvert(i)-- to screen format
	if i == nil then print("LOAD FAILURE") return null end
