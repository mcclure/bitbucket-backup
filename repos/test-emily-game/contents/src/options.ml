...
Options:|})
    ...
    in let executeArgs = [ (* Basic arguments *)
        ...
        ("--", Arg.Unit(fun () -> (* Special for game-- "for program" args go after -- *)
            raise ArgPlus.Complete
        ), ""); (* No summary, this shouldn't be listed with options. *)

        (* Args *)
        ("-e", Arg.String(fun f ->
            target := Some(Literal f);
            raise ArgPlus.Complete
        ), "Execute code inline");
    ...
    in let targetParse t = (target := Some(File t); raise ArgPlus.Complete)
    ...
    ArgPlus.argParse args targetParse usage (fun progArgs ->
        ...
        if run.printVersion then print_endline fullVersion else (
            run.args <- progArgs;
            run.target <- (match !target with (* Special behavior for emily-game *)
                | None -> Some ( File(Path.executableRelativePath "game/game.em") )
                | _    -> !target);
