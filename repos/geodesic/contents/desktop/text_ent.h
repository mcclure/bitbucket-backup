...
struct scroller_ent : public ent {
	...
	virtual void pushSplitLine(const string &text) {	
		string first = text;
		while (1) {
			string rest;
	...
	virtual void pushLine(const string &text) { // FIXME: Non-integrated approach vulnerable to "\n" right after linesplit
		string remaining = text;
		for(int c = 0; c < remaining.size(); c++) {
			if (remaining[c] == '\n') {
				pushSplitLine(remaining.substr(0, c));
				remaining = c+1<remaining.size()?remaining.substr(c+1):string();
				c = -1; // Reset loop
			}
		}
		pushSplitLine(remaining);
	}
	void display(drawing *d) {
		jcImmediateColor4f(1.0,1.0,1.0,1.0);	// For debug
...
inline string str_toupper(const string &text) {
	string outline;
...
struct capital_scroller_ent : public scroller_ent {
	...
	void pushLine(const string &text) {
		scroller_ent::pushLine( str_toupper( text ) );
	}
};
