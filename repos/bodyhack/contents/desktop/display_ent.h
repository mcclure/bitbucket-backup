...
typedef hash_map<display_code, quadpile *> quad_db;
...
struct drawing { // Should track framebuffer
	...
	drawing();
	~drawing();
	hash_map<display_code, quadpile *> data; // TODO: Preserve order?
	quadpile &pile(display_code code = D_C, bool _useTexture = true, bool _useColor = false, GLint _texture = 0, unsigned int _baseColor = WHITE);
