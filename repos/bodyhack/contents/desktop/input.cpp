...
void InputRules::push(bool copy) {
	InputRules *top = rules(); // Call this way to ensure 1 to start
	rulestack.push_back(copy ? InputRules(*top) : InputRules());
}

// TODO: Push down state variables also?
void InputRules::pop() {
	if (rulestack.size() > 1)
		rulestack.pop_back();
}
...
void InputRules::routeHasAxiscode(ControllerRules *controller, InputData *data, ent *target) {
	...
	switch (data->kind) {
		...
		case InputKindKeyboard: {
			if (keyboard)
				routeAttempt(AXISCODE_META(data->axiscode), touch, data, target); // Similar weird hack, but we want SDLK == 0
...
void InputRules::routeAttempt(uint32_t axiscode, uint32_t inputcode, InputData *data, ent *target) {
	...
	if ((data->axiscode & AXISCODE_LOW_MASK) && !(data->axiscode & AXISCODE_HIGH_MASK))
		data->strength *= -1;
...
#define HYPERERR(...)
#define BWRITE(x) (sizeof(x) == fwrite((char *) &(x), 1, sizeof(x), f))
#define BREAD(x) (sizeof(x) == fread((char *) &(x), 1, sizeof(x), f))
#define DEFAULTDAT (PROGDIR "controls.obj")
#define BINPUTMAX 256
#define BVERSION 0x1

inline void s_writes(FILE *f, const string &s) {
	uint32_t tempCount = s.length();
	BWRITE(tempCount);
	fwrite(s.c_str(), 1, tempCount, f);
}

inline string s_reads(FILE *f) {
	uint32_t tempCount;
	BREAD(tempCount);
	int readto = min<int>(BINPUTMAX,tempCount);
	char filename[BINPUTMAX]; // Wait WTF, no
...
void InputRules::load(FILE *f) {
	uint32_t temp = 0;
	BREAD(temp);
	if (temp != BVERSION) return;
...
void InputRuleSpec::load(FILE *f) {
	uint32_t temp = 0;
	BREAD(temp);
	kind = (InputKind)temp;
...
void InputRules::load(const string &path) {
...
void InputRules::save(FILE *f) {
	uint32_t temp = BVERSION; // Version
	BWRITE(temp);
...
void InputRuleSpec::save(FILE *f) {
	uint32_t temp;
	
	temp = kind;
	BWRITE(temp);
	
	s_writes(f, devicename);
	BWRITE(axiscode);
	BWRITE(inputcode);
}
void InputRules::save(const string &path) {
