...
quadpile &drawing::pile(display_code code, bool _useTexture, bool _useColor, GLint _texture, unsigned int _baseColor) {
	quadpile *&result = data[code];
	if (!result) {
...
void splatter::display(drawing *d) {
	...
	quadpile &into = d->pile(src->code, true, false, src->outTexture->texture);
...
void boxer::display(drawing *d) {
	cpVect from = cpvadd(cpvscale(size, cpv(-1, 1)), offset), to = cpvadd(cpvscale(size, cpv(1, -1)), offset);
	cpVect verts[4] = RECT(from, to);
	quadpile &into = d->pile(D_P, false, false);
	into.push4(verts);
}