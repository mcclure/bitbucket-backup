...
struct project_bridge {
	...
	ScreenImage *screenImageFromTexture(Texture *t);
	ScreenImage *screenImageFromImage(Image *i);
	void loadTextureFromImage(ScreenMesh *m, Image *i);
	void threshold(Image *i, float t, Color c1, Color c2);
};
