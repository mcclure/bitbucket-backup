...
ScreenImage *project_bridge::screenImageFromTexture(Texture *t) {
	return new ScreenImage(t);
}

ScreenImage *project_bridge::screenImageFromImage(Image *i) {
	return new ScreenImage(i);
}

void project_bridge::loadTextureFromImage(ScreenMesh *m, Image *i) {
	m->loadTexture(i);
}

void project_bridge::threshold(Image *i, float t, Color c1, Color c2) {
