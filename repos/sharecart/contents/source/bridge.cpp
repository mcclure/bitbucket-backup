...
bool project_bridge::filedump_external_out(String _path, String data) {
	_path = PROGDIR + _path;
...
BSound::BSound() : PSound(), pitch(440), theta(0), volume(1) {
	debug_audio_init();
	memset(param, 0, sizeof(param));
	memset(theta2, 0, sizeof(param));
}
...
#define BPCLIP (param[0])
// PARAM 1 MUST STAY UNUSED
#define BCRFROM 2
						  ...
						  PaStreamCallbackFlags)
{
	...
	float *out = (float*)outputBuffer;
	double thetaoff = pitch / 44100.0 * M_PI*2;
	double theta2off[BCRLEN];
	double truevolume = volume * BPCLIP;
	
	for(int c = 0; c < BCRLEN; c++) {
		theta2off[c] = param[BCRFROM+c] * pitch / 44100.0 * M_PI*2;
	}
	
	for(unsigned long i=0; i<framesPerBuffer; i++)
	{
		double v = sin(theta)*truevolume;
		theta += thetaoff;
		
		// FM synth
		for(int c = 0; c < BCRLEN; c++) {
			v *= cos(theta2[c]);
			theta2[c] += theta2off[c];
		}
