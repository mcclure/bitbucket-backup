...
room_auto::room_auto(const string &_spec, bool _fake) : automaton(), screen(NULL), scene(NULL), spec(_spec), fake(_fake), a(NULL), b(NULL) {
}
...
void room_auto::loadFromFile(Object &overlay, const string &filename) {
	if (endsWith(filename, ".xml")) {
		...
		roomloader *svg;
		if (screenClass == "PhysicsScreen") {
#if PHYSICS2D
			svg = new physics_svgloader(this, &overlay, (PhysicsScreen *)screen);
#endif										
		} else {
			svg = new roomloader(this, &overlay, screen);
		}
		svg->load(filename);
		...
		ERR("ERROR: Don't know how to load '%s", filename.c_str());
	}
}
...
void room_auto::loadFromOverlay(Object &overlay) {
	...
	if (temp = overlay.root["onUpdate"]) {
		onUpdate.push_back( terminal_auto::singleton()->compile(temp->stringVal.getSTLString()) );
	}
...
bool roomloader::loadRootXml(TiXmlElement *xml, const svgtransform &parent_transform) {
	...
	if (temp)
		owner->onUpdate.push_back( terminal_auto::singleton()->compile(temp) );
...
bool roomloader::localAddChild(ScreenEntity *shape, TiXmlElement *xml, svgtype kind, const svgtransform &transform) {
	const char *onCollideText = Attribute(xml, "onCollide");
	storedfunc *onCollide = onCollideText ? terminal_auto::singleton()->compile(onCollideText) : NULL;
	if (onCollide) 
		owner->onCollide[shape] = onCollide;
	
	int isStatic = false; // Different from normal
	xml->QueryIntAttribute("polycode:isStatic", &isStatic);	
	const char *id = xml->Attribute("id");
	if (id) {
		if (!isStatic || string("text") == xml->Value()) {
			owner->obj[id] = shape;
		}
