...
class GitHandler(object):
    ...
    def upload_pack(self, remote, revs, force):
        ...
        def changed(refs):
            self.known_remote_refs = refs # This will be needed later by gitrepo.listkeys
    ...
    def get_changed_refs(self, refs, revs, force):
        ...
        for rev in revs:
            ...
            if not (heads or tags):
                # Somehow, we're pushing a nonexistent revision. If the revision is the special "all zero"
                # revision, this is normal and we should silently skip it. Otherwise, throw an error.
                if rev == GitHandler.null_revision:
                    continue
...
# This is a special hash code hg uses for empty repositories and nonexistent bookmarks.
GitHandler.null_revision = ("\0"*20)
GitHandler.null_revision_hex = ("00"*20)