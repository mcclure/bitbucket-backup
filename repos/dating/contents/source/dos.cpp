...
//		ERR("At %d,%d VERTEX %lf,%lf,%lf,%lf TEX %lf,%lf,%lf,%lf", x, y, pos[0],pos[1],pos[2],pos[3], tex[0],tex[1],tex[2],tex[3]);
	...
	virtual void Render(Camera *targetCamera = NULL) {
		...
		if (source->gfx_dirty) {
			//CoreServices::getInstance()->getRenderer()->setTextureFilteringMode(Renderer::TEX_FILTERING_NEAREST);
			delete gfx;
			gfx = CoreServices::getInstance()->getRenderer()->createTexture(source->w, source->h, (char *)&source->px(0,0), true, false);
			if (!gfx_mesh) {
				gfx_mesh = new Mesh(Mesh::QUAD_MESH);
				Polycode::Polygon *p = new Polycode::Polygon;
				
				double pos[4] = {TAL, TAU, 0, 0};
				pos[2] = pos[0] + 288*col40factor; pos[3] = pos[1] + TAFY*DOS_LINE_ROWS;
...
void gfxcontainer::pxfill_blend(unsigned int v, int _dstx, int _dsty, int dx, int dy) { // SO MUCH CODE DUPLICATION!
	if (_dstx < 0) { dx += _dstx; _dstx = 0; } // TODO: Minifix
	if (_dsty < 0) { dy += _dsty; _dsty = 0; }
	if (dx < 0) { dx = w; }
	if (dy < 0) { dy = h; }
	dx = ::min(dx, w - _dstx);
	...
	unsigned char *bsrc = (unsigned char *)&v;
	float fsrc[4];
	for(int c = 0; c <= 3; c++) {
		fsrc[c] = bsrc[c]/256.0;
	}
...
// BRESENHAM
void gfxcontainer::pxline(unsigned int v, int bx, int by, int tx, int ty) {	
	int x = bx, y = by; // Iterator
	int dx,dy,sx,sy; // Distance x, sign x
	
	signsplit(tx, bx, dx, sx);
	signsplit(ty, by, dy, sy);
...
void gfxcontainer::pxcopy_blend(const gfxcontainer *src, int _dstx, int _dsty, int _srcx, int _srcy, int dx, int dy, bool xflip, bool yflip) {
	if (!fix(src,_dstx,_dsty,_srcx,_srcy,dx,dy)) return;
	
	int _mod_srcx = (xflip ? _srcx + dx - 1 : _srcx), modx = (xflip ? -1 : 1);
	int _mod_srcy = (yflip ? _srcy + dy - 1 : _srcy), mody = (yflip ? -1 : 1); 
	
	for(int y = 0; y < dy; y++) {
		int dsty = _dsty + y;
		int srcy = _mod_srcy + y*mody;
		for(int x = 0; x < dx; x++) {
			int dstx = _dstx + x;
			int srcx = _mod_srcx + x*modx;
...
void gfxcontainer::pxmix_ryb(const gfxcontainer *rg, const gfxcontainer *gg, const gfxcontainer *bg) {
	//							       r     y     ry    b     rb    yb    ryb
	const unsigned char rt[8] = {0x00, 0xFF, 0xEF, 0xFF, 0x00, 0x9F, 0x00, 0xFF};
	const unsigned char gt[8] = {0x00, 0x00, 0xEF, 0x9F, 0x00, 0x00, 0xFF, 0xFF};
	const unsigned char bt[8] = {0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0xFF};
	const unsigned char at[2] = {0x00, 0xFF};
	
	for(int y = 0; y < h; y++) {
		for(int x = 0; x < w; x++) {
			unsigned char *rsrc = (unsigned char *)&rg->px(x,y);
			unsigned char *gsrc = (unsigned char *)&gg->px(x,y);
			unsigned char *bsrc = (unsigned char *)&bg->px(x,y);
			unsigned char *bdst = (unsigned char *)&px(x,y);
			
			unsigned int r = bool(rsrc[0]);
			unsigned int g = bool(gsrc[1])*2;
			unsigned int b = bool(bsrc[2])*4;
			unsigned int a = rsrc[3] || gsrc[3] || bsrc[3];
...
void gfxcontainer::pxcopy_rgb(const gfxcontainer *src, int _mask, int _dstx, int _dsty, int _srcx, int _srcy, int dx, int dy) {		
	if (!fix(src,_dstx,_dsty,_srcx,_srcy,dx,dy)) return;
	
	uint32_t mask = ALPHA; // Alpha
	if (_mask & 1) mask |= 0x000000FF; // Red
	if (_mask & 2) mask |= 0x0000FF00; // Green
	if (_mask & 4) mask |= 0x00FF0000; // Blue
	uint32_t unmask = (mask ^ INVERT) | ALPHA; // Erase existing but leave alpha
	...
	for(int y = 0; y < dy; y++) {
		...
		for(int x = 0; x < dx; x++) {
			...
			// Notice we subtly "swap" dst and src here so src alpha is used w/o copying
			uint32_t *bsrc = (uint32_t *)&src->px(srcx,srcy);
			uint32_t *bdst = (uint32_t *)&px(dstx,dsty);
...
void gfxcontainer::pxcopy_text(const gfxcontainer *src, String text, int _dstx, int _dsty, bool inv, int _gridx, int _gridy, int _kernx, int _kerny) {
	pxcopy_text(src, text.getSTLString(), _dstx, _dsty, inv, _gridx, _gridy, _kernx, _kerny);
}

void gfxcontainer::pxcopy_text(const gfxcontainer *src, string text, int _dstx, int _dsty, bool inv, int _gridx, int _gridy, int _kernx, int _kerny) {
	int gridx = _gridx < 0 ? src->w / 16 : _gridx;
	int gridy = _gridy < 0 ? src->h / 16 : _gridy;
	int kernx = _kernx < 0 ? gridx : _kernx;
//	int kerny = _kerny < 0 ? gridy : _kerny;
	uint8_t high = inv ? 0x80 : 0x00;
	...
	for(int c = 0; c < text.length(); c++) {
		...
		int x = ch % 16;
		int y = ch / 16;
		int dstx = _dstx + c*kernx;
		int dsty = _dsty;
		pxcopy(src, dstx, dsty, x*gridx, y*gridy, gridx, gridy);
	}
}
...
bool gfxcontainer::load_image(string filename, bool resize) {
	Image temp(filename);
	bool result = false;
					if		(temp.isLoaded()) {
		if (resize)
...
void type_automaton::rinse_offset() {
	...
	int xbase = 288, ybase = 8*DOS_LINE_ROWS;
...
void type_automaton::fill(int x, int y, int dx, int dy, String s, bool inv) {
	fill(x,y,dx,dy, s.size()>0 ? s[0] : DOS_BLANK, inv);
}
void type_automaton::fill(int _x, int _y, int dx, int dy, unsigned char s, bool inv) {
