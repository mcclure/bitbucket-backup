...
struct gfxcontainer {
	...
	void pxfill_blend(unsigned int v, int _dstx = 0, int _dsty = 0, int dy = -1, int dx = -1);
	void pxline(unsigned int v, int bx, int by, int tx, int ty);
	...
	void pxcopy_blend(const gfxcontainer *src, int _dstx = 0, int _dsty = 0, int _srcx = 0, int _srcy = 0, int _dx = -1, int dy = -1, bool xflip = false, bool yflip = false);
	void pxcopy_rgb(const gfxcontainer *src, int _mask = 7, int _dstx = 0, int _dsty = 0, int _srcx = 0, int _srcy = 0, int _dx = -1, int dy = -1);
	void pxcopy_text(const gfxcontainer *src, String text, int _dstx = 0, int _dsty = 0, bool inv = false, int _gridx = -1, int _gridy = -1, int _kernx = -1, int _kerny = -1);
	void pxcopy_text(const gfxcontainer *src, string text, int _dstx = 0, int _dsty = 0, bool inv = false, int _gridx = -1, int _gridy = -1, int _kernx = -1, int _kerny = -1);
	...
	void pxmix_ryb(const gfxcontainer *rg, const gfxcontainer *gg, const gfxcontainer *bg);
	void pxswap(gfxcontainer *src, int _dstx = 0, int _dsty = 0, int _srcx = 0, int _srcy = 0, int _dx = -1, int dy = -1);
	bool load_image(String filename, bool resize=true) { return load_image(filename.getSTLString(), resize); }
	bool load_image(string filename, bool resize=true);
	void save_image(string filename) const;
	void init(int _w, int _h);
	gfxcontainer(int _w = 288, int _h = 192); // DOS_GFX_COLS, DOS_GFX_ROWS
};
...
struct type_automaton : public automaton, public gfxcontainer {
	...
	bool screen_dirty; bool offset_dirty;
	int offx; int offy;
	void clear() {
	...
	void set_centered(int x, int y, int w, string s, bool clearfirst = true, bool inv = false);
	void fill(int x=0, int y=0, int dx=DOS_LINE_COLS, int dy=DOS_LINE_ROWS, String s = String(), bool inv=false);
	void fill(int x=0, int y=0, int dx=DOS_LINE_COLS, int dy=DOS_LINE_ROWS, unsigned char s = DOS_BLANK, bool inv=false);
