...
function Layout:push(t, proto, asif)
	...
	elseif asif == "trigger" then -- Button
		local input = r(UIButton(t[1], 90, 20))
		table.insert(self.inp, {input, xoff=t.xoff or 0, yoff=4} ) -- NO NO NO!! On the xoff
		...
		t = tableMerge(tableMerge({},proto),t)
		...
		elseif asif == "sliderex" then
			...
			function setFixedText(into, value, round)
				into:setText(string.format(round and "%d" or "%0.2f", value))
			end
			
			local default = t.default and tonumber(t.default)
			if t.default then setFixedText(input, t.default, t.round) slider:setSliderValue(t.default) end
			
			table.insert(self.inp, {slider, xoff=basexoff, ystill=true} )
			function onSlider(ctx,event)
				ctx[1]:onSlider(ctx, event)
				
				function mate(obj, value) setFixedText(obj, value, ctx.round) end
				ctx[1]:onSlider( {self, target=ctx.partner, act=mate}, event )
			end
			slider:addEventListener( {self, target=t.target, act=t.act, partner=input, round=t.round}, onSlider, UIEvent.CHANGE_EVENT)

			table.insert(self.inp, {input, xoff=basexoff+slidwid+12, yoff=t.ystill and 0 or -8, yafter=t.yafter, ystill=t.ystill} )
