...
struct TypeScene : public Scene {
	...
	Mesh *gfx_mesh;
	Texture *gfx_texture;
	TypeScene(type_automaton *_source, bool _virtualScene = false) : Scene(_virtualScene), source(_source), gfx_mesh(NULL), gfx_texture(NULL) {
	...
	virtual void Render(Camera *targetCamera = NULL) {
		...
		if (source->gfx_dirty) {
			...
			if (source->fuzzy)
				...
				CoreServices::getInstance()->getRenderer()->setTextureFilteringMode(Renderer::TEX_FILTERING_NEAREST);
			gfx_texture = CoreServices::getInstance()->getRenderer()->createTexture(source->w, source->h, (char *)&source->px(0,0), true, false);
			if (!gfx_mesh) {
		...
		if (gfx_texture) {
			renderer->setVertexColor(1.0,1.0,1.0,1.0);
			renderer->setTexture(gfx_texture);
...
void gfxcontainer::pxpaint(int x, int y, int c) {
	if (within(x,y))
		px(x,y) = c;
	gfx_dirty = true;
}
...
void type_automaton::fill(int x, int y, int dx, int dy, String s) {
	fill(x,y,dx,dy, s.size()>0 ? s[0] : DOS_BLANK);
}
void type_automaton::fill(int _x, int _y, int dx, int dy, unsigned char s) {
