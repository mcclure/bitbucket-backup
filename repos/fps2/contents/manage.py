...
help += "\t%prog archive [name]\nSave a [name].zip exported version of current repository and Polycode.\n"
...
help += "\t--no-ply (when building extras, don't include ply module)"
...
help += "\t--dir=[dir] (when archiving, save result in this dir)"
help += "\n"
...
for a in ["bindings-only", "no-tools", "no-ply", "debug", "symbols"]: # Long args, flags
	parser.add_option("--"+a, action="store_true")
for a in ["target", "host", "cmake-args", "dir"]: # Long args with arguments
...
def path_for(seq):
	...
	return seq
def abspath_for(seq):
	return os.path.abspath(path_for(seq))
def p_link(to):
...
def repo_update(rev = None):
	...
	if subprocess.call(cmd, cwd=polycode_working_dir):
		...
		return 1
	return 0
def something_in_way(path):
	if os.path.exists(path):
		print "ERROR: Something already exists at path '%s'" % (path)
		return 1
	return 0
def polycode_installpath():
	return path_for([polycode_configuration, target_platform])
def release_dir():
...
if target_platform == "Darwin":
	def cmake_in(dir, extra_args = []):
		return subprocess.call(["cmake", "-G", "Xcode", "-DCMAKE_BUILD_TYPE="+polycode_configuration, "-DPOLYCODE_RELEASE_DIR="+ abspath_for(release_dir())] + extra_args + cmake_args() + [".."], cwd=dir)
	...
	def cmake_in(dir, extra_args = []):
		return subprocess.call(["cmake", "-G", "Unix Makefiles", "-DCMAKE_BUILD_TYPE="+polycode_configuration, "-DPOLYCODE_RELEASE_DIR="+abspath_for(release_dir())] + extra_args + cmake_args() + [".."], cwd=dir)
	def build_in(dir, target, makecmd = None):
...
def cmd_build():
	...
	if flag("q"):
		...
		if flag("d"):
			do_rmtree([polycode_working_dir, polycode_configuration, target_platform]) # Only clear product dir if cleaning EVERYTHING
	...
	typical_build([polycode_working_dir, build_dir()],
		(["-DPOLYCODE_USE_OPENAL=1"] if flag("openal") else ["-DPOLYCODE_USE_OPENAL=0"]) +
		...
		(["-DPOLYCODE_DEBUG_SYMBOLS=1"] if flag("symbols") else []) )
	...
	extra = ["Modules/Bindings/2DPhysics/Source/Physics2DLUA.cpp",
	         ...
	         "Modules/Bindings/UI/Source/UILUA.cpp",
	...
	if not flag("no_ply"):
		print "Copying extras: ply."
		if os.path.exists(path_for([extra_dir, "ply"])):
			print "Skipping, ply already present."
		else:
			plydir = tempfile.mkdtemp()
			if subprocess.call(["easy_install", "-eb", plydir, "ply"]):
				print "ERROR: ply download failed. If this becomes a problem, run again with --no-ply";
				sys.exit(1)
			if subprocess.call(["cp", "-R", path_for([plydir, "ply", "ply"]), extra_dir]):
				print "ERROR: ply copy failed. If this becomes a problem, run again with --no-ply";
				sys.exit(1)
	
	# That's all
	print "Done."
	
# Returning a string indicates success, returning a number indicates failure.
def get_desired_path():
	if not have_hg():
		return 1
...
def cmd_load(): # TODO: Take an argument
	desired_path = get_desired_path()
	if desired_path is 1:
		return 1
...
def cmd_archive(name):
	if len(name) <= 0:
		print "ERROR: Zero length name? How did you even do that."
		return 1
	...
	if dir:
		mkdir(dir)
		finalname = dir + "/" + finalname
		finalzipname = dir + "/" + finalzipname
	for path in (name, zipname,finalname, finalzipname):
		if something_in_way(path):
			return 1
	...
	if subprocess.call(["hg", "archive", name]):
		print "ERROR: Failed to archive repository."
		return 1
	if subprocess.call(["cp", "-R", desired_path, name + "/Polycode"]):
		print "ERROR: Failed to copy Polycode dir."
		return 1
	if subprocess.call(["zip", "-r", zipname, name]):
		print "ERROR: Failed to make zip file."
		return 1
		
	if dir:
		if subprocess.call(["mv", name, finalname]):
			print "ERROR: Failed to move directory to final location."
			return 1
		if subprocess.call(["mv", zipname, finalzipname]):
			print "ERROR: Failed to move zip file to final location."
			return 1
...
if len(cmds) == 1 or (len(cmds) == 2 and cmds[0] == 'archive'):
	arg = cmds[0]
	if arg == 'detach':
