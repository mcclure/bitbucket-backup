...
struct project_bridge {
	...
	Vector3 matrixApply(Matrix4 mat, Vector3 val);
	...
	void Audio3();
    void unfocus();
	SceneEntity *collidesWith(CollisionScene *scene, SceneEntity *test);
    String oneFilePicker(String ext);
	Entity *getFirstEntityInRay(CollisionScene *s, const Vector3 &origin,  const Vector3 &dest);
};
...
#define BPARAMCOUNT 3
#define BIPARAMCOUNT 2

#define TONEMAX 5
#define STEPMAX 5
#define BASEFREQ param[0]
#define PREAMP param[1]
#define THRESHOLD param[2]
#define TONECOUNT iparam[0]
#define TONEVARY iparam[1]
#define TONERESET 4
...
struct BSound : PSound {
	...
	double theta[TONEMAX]; double stepoff[TONEMAX]; int stepis[TONEMAX]; int steps[STEPMAX]; int stepc; int step2c;
