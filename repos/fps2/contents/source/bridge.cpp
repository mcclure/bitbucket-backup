...
Vector3 project_bridge::matrixApply(Matrix4 mat, Vector3 val) {
	return mat * val;
}
...
Entity *project_bridge::getFirstEntityInRay(CollisionScene *scene, const Vector3 &origin,  const Vector3 &dest) {
	return scene->getFirstEntityInRay(origin, dest).entity;
}
...
struct FindOneCallback : public btCollisionWorld::ContactResultCallback {
	CollisionScene *parent;
	SceneEntity *found;
	
	FindOneCallback(CollisionScene *_parent)
		: btCollisionWorld::ContactResultCallback(), parent(_parent), found(NULL) {}
...
SceneEntity *project_bridge::collidesWith(CollisionScene *scene, SceneEntity *test) {
	CollisionSceneEntity *pse = scene->getCollisionByScreenEntity(test);
	FindOneCallback callback(scene);
	
	pse->lastPosition = test->getPosition(); // Maybe it's moved?
	pse->Update();
	
	scene->world->contactTest(pse->collisionObject, callback);
...
BSound::BSound() : PSound(), pitch(1), volume(1), stepc(0), step2c(0)
{
	debug_audio_init();
	memset(param, 0, sizeof(param));
    memset(iparam, 0, sizeof(iparam));
    memset(theta, 0, sizeof(theta));
	for(int c = 0; c < TONEMAX; c++)
		stepoff[c] = 1;
    memset(stepis, 0, sizeof(stepis));
						  ...
						  PaStreamCallbackFlags)
{
	...
	{		
		...
		if (stepc >= TONEVARY) {
			for(int c = 0; c < TONECOUNT && c < TONEMAX; c++) {
				if (step2c%(c+1) == 0) {
					if (stepis[c] >= TONERESET) {
						stepis[c] = 0;
						stepoff[c] = 1;
					} else {
						stepoff[c] *= pow(pow(2, 1/12.0), steps[stepis[c]]);
						stepis[c]++;
					}
				}
			}
		...
		if (BASEFREQ) {
			for(int c = 0; c < TONECOUNT && c < TONEMAX; c++) {
				v += sin(theta[c]);
				theta[c] += (2*M_PI*BASEFREQ * stepoff[c]) / 44100;
			}
		}
