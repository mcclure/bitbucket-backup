...
#define NAME_OF_THIS_PROGRAM "LD21"
...
struct block_info : public live_info {
	block_info() : live_info(C_WALL, C_SOLIDS|C_SEP), dragging(false), lock(0), siren(NULL), wantp(cpvzero), wantp_s(cpvzero), draggingInto(cpvzero), updated(-2), updated_s(-2) {}
	bool dragging;
	unsigned int lock;
	fluttertone *siren;
