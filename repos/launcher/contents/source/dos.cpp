...
void gfxcontainer::broken_pxcopy(const gfxcontainer *src, int _dstx, int _dsty, int _srcx, int _srcy, int _dx, int dy) {
	int dx = _dx;
	int dstx = _dstx; if (dstx < 0) { dx += dstx; dstx = 0; }
	int srcx = _srcx; if (srcx < 0) { dx += srcx; srcx = 0; }
	dx = ::min(dx, w - dstx);
	dx = ::min(dx, w - srcx);
	if (dx <= 0) return;
	...
	for(int y = 0; y < dy; y++) {
		int dsty = _dsty + y;
		int srcy = _srcy + y;
		if (dsty < 0 || srcy < 1) continue;
		if (dsty >= h || srcy >= h) break;
		memmove(&px(dstx,dsty), &px(srcx, srcy) - dx/8, sizeof(gfx[0])*dx);
	}
	gfx_dirty = true;
}
...
type_automaton::type_automaton(bool _yflip) : automaton(), gfxcontainer(), display(NULL), screen_dirty(true), offset_dirty(true), offx(0), offy(0), yflip(_yflip) {
	clear();
}
				...
				if (was_lctrl || was_rctrl) {
					...
					if (lctrl || rctrl) {
						...
						if (code == KEY_F1) {
							...
							if (code == KEY_RETURN)
								x = 0;
						} else if (IS_MODIFIER(code) || code == KEY_ESCAPE || code == KEY_F4 || code == KEY_TAB || !inputEvent->charCode) {
							// DO NOTHING!!!
						} else {
