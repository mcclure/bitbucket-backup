...
void project_bridge::loadPak(String name) {
	reloadPak(name.getSTLString());
	if (name.size())
		load_room_txt("media/init.txt");
	else
		load_room_txt("media/menu.txt"); // Returning to main menu
}

Number project_bridge::globalLastSize() {
	extern double globalLastSize;
	return globalLastSize;
}
