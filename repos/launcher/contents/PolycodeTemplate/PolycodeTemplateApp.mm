...
void unloadPak() {
#if SELF_EDIT
	PHYSFS_removeFromSearchPath (currentPak.c_str());
#else
	CoreServices::getInstance()->getResourceManager()->removeArchive(currentPak);
#endif
}

void loadPak(const string &name = string()) {
#if SELF_EDIT
	{
...
#define MEDIA_DEPTH 3
		...
		currentPak = cwd;
		if (!name.empty())
			currentPak = currentPak + "/sub/" + name;
		selfedit_dir = currentPak;
		ERR("Will look for media dir in %s\n", currentPak.c_str());
		PHYSFS_addToSearchPath (currentPak.c_str(), true);
		free(cwd);
	}
#else
	if (name.empty()) {
		currentPak = FP"media.pak";
	} else {
		currentPak = FP + name + ".pak";
	}
	
	CoreServices::getInstance()->getResourceManager()->addArchive(currentPak);
#endif
}

void reloadPak(string name) {
	unloadPak();
	loadPak(name);
}
...
#if defined(__APPLE__)
	core = new CocoaCore(view, surface_width,surface_height,fullscreen,aa, 0,0,60);
#elif defined(_WINDOWS)
	core = new Win32Core(view, surface_width,surface_height,fullscreen,aa, 0,0,60);
#else
	core = new SDLCore(view, surface_width,surface_height,fullscreen,aa, 0,0,60);
#endif
	cor = core;
