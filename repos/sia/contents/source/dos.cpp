...
#define TAU (col40yo)
#define TAL (col40xo)
#define TTU (0 + source->dg[DG_YO])
#define TTL (0 + source->dg[DG_XO])
#define TTFX (7.0/128 * source->dg[DG_SPAN])
#define TTFY (1.0/16)
...
type_automaton::type_automaton() : automaton(), gfxcontainer(), display(NULL), screen_dirty(true), offset_dirty(true), offx(0), offy(0) {
	dg[DG_SPAN] = 1; dg[DG_XO] = 0; dg[DG_YO] = 0;
	clear();
}
...
void type_automaton::glitch(Number a, Number b, Number c) {
	dg[DG_SPAN] = a;
	dg[DG_XO] = b;
	dg[DG_YO] = c;
	offset_dirty = true;
